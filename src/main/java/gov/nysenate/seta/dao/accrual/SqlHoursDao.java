...
/**
 * Created by heitner on 8/29/2014.
 */
...
/*                  "AND EXISTS (SELECT 1\n" +
    ...
    public BigDecimal getTotalHours(int empId, int year) {
        logger.info("SqlHoursDao", "~~~~~~~~~~~~~~~~~~~~~~~(a)getTotalHours start");
        return getTotalHours(empId, new LocalDate(year, 1, 1).toDate(), new Date());
    }

    @Override
    public BigDecimal getTotalHours(int empId, Date beginDate, Date endDate) {
        logger.debug("~~~~~~~~~~~~~~~~~~~~~~~(b)getTotalHours start");
